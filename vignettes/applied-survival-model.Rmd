...
```{r get-stan-code}
stan_code_url <- 'https://raw.githubusercontent.com/to-mi/stan-survival-shrinkage/master/wei_gau.stan'
...
Next, we want manually specify the initial values Stan will use when fitting the model. 

It's important to hard-code these since many of our parameters (e.g. variance parameters) are constrained to be positive, so the default initial values (which range from -2 to 2) will not be ideal.

The model authors provided us some example code ([example.R](https://github.com/to-mi/stan-survival-shrinkage/blob/master/example.R#L88-L103)) which we will modify to wrap in a function.

Stan can either take initial values in the form of a function (as we use here) which will be called for each chain, a list of lists (one per chain), or if only one chain is used a simple list of inputs. In any of the above scenarios, `stan` is very particular about the dimension & type of inputs given.

Here is the function we define (technically a functional):
...
```{r stan-test-fit}
test_fit <- rstan::stan(model_code = stan_code,
                        data = stan_data,
                        iter = 10,
                        chains = 1,
                        init = make_inits(1, 1))
