...
In these examples, we will be using models which have **previously been checked against simulated data**, and whose properties are fairly well understood. Those data checks are provided as separate vignettes. See those for details.

This vignette, by comparison, focuses on the practical work of fitting the models to a clinical dataset.

# Outline

Our analysis will iterate through various models and approaches to analysis. 

We will start by inspecting our data and doing some cursory exploration.

Then, we iterate:

0. Select a model for consideration
    - inspect model
    - test model on simulated data
    - fit model to clinical data
    - Check model convergence
    - Check posterior predictive distributions vs actual data
    - Summarize ELPD using `loo`
    - Compare model to previous models
7. Propose an update to the model / next thing to try
...
Load the clinical data for this cohort:
...
Let's do some minimal data manipulation on this DataFrame to make it easier to work with. We will save the converted data in a data.frame called `clin_data`.
...
In survival analysis, the outcome or dependent variable is **time to event**.  

Our first analysis will treat **overall survival** as the event of interest. In this cohort, the overall survival is described by two variables: `os_status` & `os_months`. 

We will start by inspecting these data.

### Negative/missing event times

We have one observation with missing data for Overall Survival
...
Use the `survfit` function to plot the KM survival curve 
...
But in this case I'm also looking to see whether our posterior intervals contain the parameters used to simulate our dataset. 

We will do a quick review of the traceplots before continuing with our posterior predictive checking.
